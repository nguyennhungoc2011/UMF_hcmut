...
def errMetric(realVec, predVec, metrics):
    ...
    for metric in metrics:
        ...
        if 'MRE' == metric or 'NPRE' == metric:
            ...
            if 'NPRE' == metric:
                index = int(np.floor(0.9 * relativeError.shape[0]))
                try:
                    float_value = float(relativeError[index])
                except ValueError:
                    logger.error('Cannot convert to an float')
                result = np.append(result, float_value)
    return result
